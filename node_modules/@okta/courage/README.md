...
You can use the playground to learn about and try out the courage framework.

1. Start the playground server with a specific port. If a port is not specified, the server will try to start at port 3000 by default.

    ```bash
    [~]$ cd $OKTA_HOME/courage
    [courage]$ grunt play --port=3000
    ```

2. Load the playground from a web browser

   The playground server listens to port 3000 by default. Go to [http://localhost:3000/form](http://localhost:3000/form)
