...
function (Okta, Util, OAuth2Util, Enums, BrowserFeatures, Errors, ErrorCodes) {
  ...
  fn.routeAfterAuthStatusChange = function (router, err, res) {
    ...
    case 'SUCCESS':
      ...
      // account unlock success page
      var lastRes = router.appState._previousAttributes.lastAuthResponse;
      if (lastRes.status == 'RECOVERY' && lastRes.recoveryType === Enums.RECOVERY_TYPE_UNLOCK) {
