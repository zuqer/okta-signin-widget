  ...
  return BaseLoginController.extend({
    ...
    initialize: function() {
      ...
      var Schema = RegistrationSchema.extend({
        preRender: this.options.settings.get('registration.preRender'),
        url: this.options.settings.get('baseUrl')+'/api/v1/registration/'+clientId+'/form'
      });
      
      var schema = new Schema();
      var localeUtil = Okta.loc;
      ...
      var postSchemaFetch = _.bind(function(){
        ...
        this.listenTo(this.model, 'parseFieldLevelErrors', _.bind(function (fieldError, callback) {
          callback(localeUtil(fieldError.errorToken));
        }, this.model));
        ...
        properties.each(function(schemaProperty) {
          ...
          if (inputOptions.options && subSchemas && subSchemas._type === 'oneOf') {
            inputOptions.options = subSchemas.reduce(function(memo, subSchema){
              memo[subSchema.get('const')] = subSchema.get('title');
              return memo;
            }, {});
          }
