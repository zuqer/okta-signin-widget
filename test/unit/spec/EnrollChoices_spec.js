...
function (Okta, OktaAuth, Util, EnrollChoicesForm, Beacon, Expect, Router,
  ...
  Expect.describe('EnrollChoices', function () {
    ...
    function setupMultipleU2FEnrollments (status, enrollment, policyEnrollment) {
      var res = deepClone(resAllFactors);
      res.response._embedded.factors[10].status = status;
      res.response._embedded.factors[10].enrollment = enrollment;
      res.response._embedded.factors[10].policy.enrollment = policyEnrollment;
      return setup(res);
    }

    function setupMultipleU2FWithActiveStatusAndOptionalEnrollment (policyEnrollment) {
      return setupMultipleU2FEnrollments('ACTIVE', 'OPTIONAL', policyEnrollment);
    }

    function setupMultipleU2FWithNotSetupStatusAndRequiredEnrollment (policyEnrollment) {
      return setupMultipleU2FEnrollments('NOT_SETUP', 'REQUIRED', policyEnrollment);
    }

    function setupMultipleU2FWithActiveStatusAndRequiredEnrollment (policyEnrollment) {
      return setupMultipleU2FEnrollments('ACTIVE', 'REQUIRED', policyEnrollment);
    }
